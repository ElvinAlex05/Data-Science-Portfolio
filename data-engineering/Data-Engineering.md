...
**Evidence:**<img width="1865" height="791" alt="image" src="https://github.com/user-attachments/assets/de9da16f-dfc8-4e2f-93ed-dfd112cdec26" />
...
A set of sanity checks was used to confirm the ETL produced analysis-ready extracts. Output previews confirm both extracts contain the expected schema and include the engineered `Total Price` measure, and that the pipeline successfully writes `.hyper` outputs for Tableau Desktop consumption. The cleaning step evidence confirms that null `CustomerID` records are excluded and that `Quantity` is constrained to a valid range, reducing the likelihood of invalid transactions biasing revenue KPIs. UK scoping is validated by the country filter applied in the UK branch, and the effectiveness of product description consolidation is evidenced by the documented reduction in distinct description values from **1,093** to **608** . Together, these checks provide confidence that the outputs are consistent, interpretable, and fit for customer segmentation and product performance reporting.
